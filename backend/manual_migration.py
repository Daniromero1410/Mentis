...
with engine.connect() as conn:
    # 1. Tareas Prueba TO
    try:
        print("Agregando observaciones_fotograficas a tareas_prueba_to...")
        conn.execute(text("ALTER TABLE tareas_prueba_to ADD COLUMN observaciones_fotograficas VARCHAR;"))
        conn.commit()
        print("Columna agregada exitosamente a tareas_prueba_to.")
    except Exception as e:
        print(f"Error o la columna ya existe en tareas_prueba_to: {e}")
