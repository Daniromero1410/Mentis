"""
Modelos para el módulo de AE de Trabajo
"""
...
class EstadoAE(str, Enum):
    """Estados posibles de una AE de trabajo"""
    BORRADOR = "borrador"
    COMPLETADA = "completada"


class DimensionRiesgo(str, Enum):
    """Dimensiones de riesgo psicosocial en AE de trabajo"""
...
# ===== MODELO PRINCIPAL =====
class AnalisisExigenciaMental(SQLModel, table=True):
    """Modelo principal de AE de Trabajo"""
    __tablename__ = "analisis_exigencias_mental"

    id: Optional[int] = Field(default=None, primary_key=True)

    # Control y estado
    estado: EstadoAE = Field(default=EstadoAE.BORRADOR)
    fecha_creacion: datetime = Field(default_factory=datetime.utcnow)
    fecha_actualizacion: datetime = Field(default_factory=datetime.utcnow)
    fecha_finalizacion: Optional[datetime] = None

    # Usuario que creó la AE
    ...
    concepto_final: Optional["ConceptoFinalAE"] = Relationship(back_populates="AE")


# ===== DATOS DE LA EMPRESA =====
class DatosEmpresaAE(SQLModel, table=True):
    """Datos de la empresa en la AE de trabajo"""
    __tablename__ = "datos_empresa_AE"

    id: Optional[int] = Field(default=None, primary_key=True)
    AE_id: int = Field(foreign_key="analisis_exigencias_mental.id", unique=True)
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="datos_empresa")


# ===== DATOS DEL TRABAJADOR =====
class TrabajadorAE(SQLModel, table=True):
    """Datos del trabajador evaluado"""
    __tablename__ = "trabajadores_AE"
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="trabajador")
...
class DatosEvaluadorAE(SQLModel, table=True):
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="evaluador")


# ===== SECCIONES DE TEXTO LIBRE =====
class SeccionesAE(SQLModel, table=True):
    """Secciones de texto libre del formulario"""
    __tablename__ = "secciones_AE"
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="secciones")


# ===== CONDICIONES DE RIESGO (FR/EXP/INT) =====
class CondicionRiesgoAE(SQLModel, table=True):
    """Cada condición evaluada con FR, EXP, INT"""
    __tablename__ = "condiciones_riesgo_AE"

    id: Optional[int] = Field(default=None, primary_key=True)
    AE_id: int = Field(foreign_key="analisis_exigencias_mental.id")
    ...
    # Calificaciones individuales por evaluador
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="condiciones_riesgo")


# ===== RESUMEN DE FACTORES =====
class ResumenFactorAE(SQLModel, table=True):
    """Resumen de factores de riesgo por dimensión"""
    __tablename__ = "resumen_factores_AE"
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="resumen_factores")


# ===== CONCEPTO FINAL =====
class ConceptoFinalAE(SQLModel, table=True):
    """Concepto psicológico final y conclusiones"""
    __tablename__ = "concepto_final_AE"
    ...
    AE: Optional["AnalisisExigenciaMental"] = Relationship(back_populates="concepto_final")
