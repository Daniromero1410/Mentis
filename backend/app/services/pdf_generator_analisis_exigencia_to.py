"""
Generador de PDF para Análisis de Exigencias TO
Formato secundario dentro del ecosistema TO.
"""
import os
...
def generar_pdf_analisis_exigencia_to(
    analisis,
    identificacion,
    secciones,
    desempeno,
    ...
    filename = f"Analisis_Exigencias_TO_{analisis.id}_{nombre_trab}_{timestamp}.pdf"
    ...
    center_text = Paragraph(
        ...
        "<b>VALORACIÓN<br/>ANÁLISIS DE EXIGENCIAS</b>",
        styles["HeaderSub"]
    )
    ...
    for idx, tarea in enumerate(tareas):
        ...
        desc_biomecanica = str(tarea.descripcion_biomecanica) if tarea.descripcion_biomecanica else ""
        req_motrices = str(tarea.requerimientos_motrices) if hasattr(tarea, "requerimientos_motrices") and tarea.requerimientos_motrices else ""
        
        desc_text = f"<b>Biomecánica:</b><br/>{desc_biomecanica}"
        if req_motrices:
            desc_text += f"<br/><br/><b>Requerimientos Motrices:</b><br/>{req_motrices}"
            
        desc_cell = Paragraph(desc_text, styles["LongText"])
    ...
    if perfil:
        ...
        def render_perfil_dic(title, dic_data):
        ...
        render_perfil_dic("Sensopercepción", perfil.sensopercepcion)
        render_perfil_dic("Motricidad Gruesa", perfil.motricidad_gruesa)
        render_perfil_dic("Motricidad Fina", perfil.motricidad_fina)
        render_perfil_dic("Armonía", perfil.armonia)
        render_perfil_dic("Cognitivos", perfil.cognitivos)
        render_perfil_dic("Psicosociales", perfil.psicosociales)
        render_perfil_dic("Laborales", perfil.laborales)
        
        elements.append(Spacer(1, 6))

    # ── SECCIÓN 9 + 10: RECOMENDACIONES + REGISTRO (juntos) ───────────
    # Use KeepTogether to ensure signatures area always appears
    # on the same page as recommendations for security
    sec8_9_elements = []

    sec8_9_elements.append(section_title_table("9   RECOMENDACIONES"))
    ...
    sec8_9_elements.append(section_title_table("10   REGISTRO"))
    sec8_9_elements.append(Spacer(1, 2))
