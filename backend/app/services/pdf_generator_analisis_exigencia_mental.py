"""
Generador de PDF para Aes de Trabajo de Esfera Mental
...
from app.models.analisis_exigencia_mental import DimensionRiesgo, NivelRiesgo
...
def generar_pdf_analisis_exigencia_mental(
    Ae,
    datos_empresa,
    trabajador,
    evaluador,
    ...
    """
    Genera PDF profesional de Ae de Trabajo
    """
    ...
    pdf_filename = f"ANALISIS_EXIGENCIAS_MENTAL_{sanitize_filename(nombre)}_{sanitize_filename(identificacion)}_{timestamp}.pdf"
    ...
    metodologia_texto = secciones.metodologia if secciones and secciones.metodologia else \
        "El siguiente instrumento establecido para la realización de Aes de Trabajo de Esfera Mental basa su estructura en el apartado del Dominio Demandas del Trabajo de la Batería de instrumentos para la evaluación de factores de riesgo psicosocial del ministerio de protección social."
    ...
    if concepto_final:
        ...
        if concepto_texto:
            story.append(crear_seccion_header("CONCLUSIONES FINALES - Ae DE TRABAJO DE ESFERA MENTAL"))
            story.append(Spacer(1, 5))
